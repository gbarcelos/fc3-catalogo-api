...
class VideoRestClientTest extends AbstractRestClientTest {
  ...
  public void givenAVideo_whenReceive5xxFromServer_shouldReturnInternalError() {
    ...
    final var expectedErrorMessage = "Error observed from videos [resourceId:%s] [status:500]".formatted(
        expectedId);
    ...
    verify(expectedRetries,
        getRequestedFor(urlPathEqualTo("/api/videos/%s".formatted(expectedId))));
  }
  ...
  public void givenAVideo_whenReceiveTimeout_shouldReturnInternalError() {
    ...
    final var expectedRetries = 2;
    final var expectedErrorMessage = "Timeout observed from videos [resourceId:%s]".formatted(
        golang.id());
    ...
    // when
    final var actualEx = Assertions.assertThrows(InternalErrorException.class,
        () -> target.videoOfId(golang.id()));

    // then
    Assertions.assertEquals(expectedErrorMessage, actualEx.getMessage());

    verify(expectedRetries,
        getRequestedFor(urlPathEqualTo("/api/videos/%s".formatted(golang.id()))));
  }
  ...
  public void givenCall_whenCBIsOpen_shouldReturnError() {
    ...
    final var actualEx = Assertions.assertThrows(CallNotPermittedException.class,
        () -> this.target.videoOfId(expectedId));
