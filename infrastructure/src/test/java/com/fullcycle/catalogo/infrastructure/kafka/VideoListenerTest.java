...
class VideoListenerTest extends AbstractEmbeddedKafkaTest {
  ...
  public void givenInvalidResponsesFromHandlerShouldRetryUntilGoesToDLT() throws Exception {
    ...
    final var message =
        Json.writeValueAsString(new MessageValue<>(
            new ValuePayload<>(systemDesignEv, systemDesignEv, aSource(), Operation.DELETE)));
    ...
    verify(videoListener, times(expectedMaxDLTAttempts)).onDLTMessage(eq(message),
        metadata.capture());
  ...
  @Test
  public void givenValidVideoWhenUpdateOperationProcessGoesOKThenShouldEndTheOperation()
      throws Exception {
    // given
    ...
    final var message =
        Json.writeValueAsString(new MessageValue<>(
            new ValuePayload<>(golangEv, golangEv, aSource(), Operation.UPDATE)));
  ...
  public void givenCreateOperationWhenProcessGoesOKShouldEndTheOperation() throws Exception {
    ...
    final var message =
        Json.writeValueAsString(
            new MessageValue<>(new ValuePayload<>(golangEv, null, aSource(), Operation.CREATE)));
  ...
  public void givenDeleteOperationWhenProcessGoesOKShouldEndTheOperation() throws Exception {
    ...
    final var message =
        Json.writeValueAsString(
            new MessageValue<>(new ValuePayload<>(null, golangEv, aSource(), Operation.DELETE)));
